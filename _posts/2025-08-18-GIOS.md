...
NEED TO COMPLETE SECTION - will I tho?

## InterProcess Communication - P3L3

IPC refers to a set of mechanisms that the operating system must support in order to permit multiple processes to interact with each other. IPC mechanisms are broadly categorized as message based or memory based.  

-	Examples of message passing based IPC mechanisms include sockets, and other OS-supported constructs like pipes or message queues.
-	The most common memory based mechanism is shared memory. This may be in the form of completely unstructured sets of pages of physical memory or memory mapped files. Speaking of files, these too can be perceived as a method for IPC: multiple processes read and write from the same file. We will talk about file systems in a separate lecture.

[ipc](/assets/img/Posts/GIOS/P3L3/img1.png)

### Message Based IPC
-	As the name implies, processes create messages and then send or receive them. The operating system is responsible for creating and maintaining the channel for passing messages among processes. This can be thought of as a buffer, FIFO queue, or other type of data structure.
-	The operating system also provides some interface (a port) to the processes so that they can pass messages via this channel. 
[ipc](/assets/img/Posts/GIOS/P3L3/img2.png)
[ipc](/assets/img/Posts/GIOS/P3L3/img3.png)

### Forms of Message Passing

#### Pipes
[ipc](/assets/img/Posts/GIOS/P3L3/img4.png)
#### Message Queues
[ipc](/assets/img/Posts/GIOS/P3L3/img5.png)
#### Socket API
[ipc](/assets/img/Posts/GIOS/P3L3/img6.png)

### Shared Memory IPC
[ipc](/assets/img/Posts/GIOS/P3L3/img7.png)
[ipc](/assets/img/Posts/GIOS/P3L3/img8.png)

### Copy vs Map 
[ipc](/assets/img/Posts/GIOS/P3L3/img9.png)

### SysV Shared Memory
[ipc](/assets/img/Posts/GIOS/P3L3/img10.png)

### SysV Shared Memory API
- The shmget() call is used to create or open a segment of the appropriate size and it return shmid. The flags include the various options like permissions. This unique identifier is the key and that is explicitly passed to the OS by the application. 
- The shmat() call attaches the shared memory segments into the virtual address space of the process. It maps them into the user address space. 
- The shmdt() call detaches the segment identified by this identifier so the virtual to physical memory mappings are no longer valid.
- The shmctl() operation passes certain commands related to shared memory segment management to the OS, which includes a command to remove a particular segment: IPC_RMID.
[ipc](/assets/img/Posts/GIOS/P3L3/img11.png)

### POSIX Shared Memory API 
-	The shm_open() and shm_close() calls explicitly open and close shared memory. In fact you can also call the regular open() and close() and the OS will figure out which file system the file comes from.
-	To attach or detach shared memory, the POSIX shared memory API relies on the mmap() and unmmap() calls to map or un-map files into the address space of a process.
-	To destroy a shared memory segment, there is an explicit unlink() operation. There is also shm_close() which only remove the file descriptor from the address space of the process. But to tell the OS to delete all of the shared memory related data structures and to free up the shared memory segment,  shm_unlink() is needed.
[ipc](/assets/img/Posts/GIOS/P3L3/img12.png)

### Shared Memory and Sync
[ipc](/assets/img/Posts/GIOS/P3L3/img13.png)
For synchronization between processes, we still must use some constructs such as mutexes and condition variables. There are a couple of options for inter-process synchronization.
-	First, one can use the exact same mechanisms in the threading libraries (e.g. pthreads). For instance, two pthreads processes can synchronize amongst each other using pthreads mutexes and condition variables that have been appropriately set.
-	In addition, the OS itself supports certain mechanisms for inter-process synchronization.
Regardless of the method, there must be mechanisms to coordinate:
-	the number of concurrent accesses to the shared memory region (e.g. mutexes provide this functionality and ensures mutual exclusion)
-	when data is available in the shared memory segment and ready to be consumed by the peer processes. This is some notification or signaling mechanisms (e.g. condition variables provide this functionality).
[ipc](/assets/img/Posts/GIOS/P3L3/img14.png)
...
// …create shm segment
seg = shmget(ftok(argv[0], 120), 1024, IPC_CREATE|IPC_EXCL));
...
### Sync for Other IPC
[ipc](/assets/img/Posts/GIOS/P3L3/img15.png)

### IPC Command line tools
-	ipcs will list all of the IPC facilities in the system. This will include all types of IPC message queues, semaphores. Passing the -m flag will display only the shared memory IPC.
-	There is also a utility ipcrm to remove an IPC construct. For shared memory, you use the m flag and you specify the shared memory identifier.

### Shared Memory Design Considerations
[ipc](/assets/img/Posts/GIOS/P3L3/img16.png)

### Shared Memory Design Considerations: How Many Segments
[ipc](/assets/img/Posts/GIOS/P3L3/img17.png)

### Shared Memory Design Considerations: Segment Size
[ipc](/assets/img/Posts/GIOS/P3L3/img18.png)
